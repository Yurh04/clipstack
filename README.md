...
- 文本记录支持“预览全文”：点击行内预览图标或右键菜单打开，可自动换行、滚动查看、选择文字和复制全文，Esc 关闭。单击记录仍只选中。

- 列表和图片右键菜单支持“删除记录”；每次删除有独立的 10 秒撤销窗口，底部按钮撤销最近一次。窗口内保留记录、备注、标签和托管图片，超时后删除；退出期间到期的记录在下次启动清理。设置中的批量清空仍不可撤销。
- 搜索框外：`⌘⇧S` 收藏／取消收藏，`⌘⌫` 删除，`⌘Z` 撤销最近一次删除；`⌘C` 复制、回车粘贴。搜索框内保持正常文本编辑。
- 搜索支持空格组合条件：`app:Chrome type:image after:7d tag:学习`；`type` 支持 `text`、`image`、`file`，`after:7d` 表示最近 7 天。含空格的短语可加双引号，例如 `app:"Google Chrome"`。
- “编辑备注与标签”可填写逗号分隔的标签，收藏页支持按标签及类型组合筛选。
- 文件／图片原路径不存在时显示失效提示，复制、粘贴和预览前也会检查路径。此版本不提供重新定位文件。

## 当前功能

- 自动记录文本、图片和文件路径，历史记录保存在本机。
- 仅在屏幕顶部菜单栏显示入口，不占用 Dock 和 App 切换器位置。
- 按全部、文本、图片、文件、收藏（⌘5）分类，支持搜索；已预览图片的 OCR 文本也可被搜索。
- 历史可收藏，收藏记录在各分类中置顶，同组按复制时间倒序排列，且不参与 500 条容量或按天清理。
- 收藏可编辑备注并按备注搜索；收藏切换后选中项跟随记录。设置中支持清空普通历史（保留收藏）或确认清空全部历史。
- 菜单栏图标左键打开面板，右键可暂停/恢复记录、打开设置或退出。
- 设置支持登录启动、按应用排除记录、7/30/90 天清理、存储占用查看和可再生缓存清理。
- 图片哈希、落盘、OCR 和缩略图生成在后台执行；缩略图与 OCR 结果采用有容量上限的磁盘缓存。
...
- 可再生缓存：`~/Library/Caches/ClipStack/`（缩略图上限 64 MB，OCR 上限 32 MB，可在设置中清理）。
- 文本直接存入 SQLite。
- 非图片文件只保存路径，不复制文件本体。
